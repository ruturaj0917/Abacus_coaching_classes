...
import { BookOpen, LayoutDashboard, LogOut, Users, ClipboardList, ShieldCheck } from "lucide-react";
...
const navItems = [
  ...
  { href: "/admin/tests", label: "Manage Tests", icon: ClipboardList },
];
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background">
      {/* Sidebar */}
      <aside className="w-64 flex-shrink-0 flex flex-col bg-card border-r border-border/50 sidebar-glow">
        {/* Brand */}
        <div className="flex items-center gap-3 px-6 py-5 border-b border-border/50">
          ...
          <div>
            <div className="font-bold text-sm text-foreground">Admin Panel</div>
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        {/* Top bar */}
        <header className="flex items-center justify-between h-14 px-8 border-b border-border/50 bg-card/30 backdrop-blur-sm sticky top-0 z-10">
